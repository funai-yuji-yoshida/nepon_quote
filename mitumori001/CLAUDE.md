...
新しい関数を呼び出すコードを追加する場合、**必ず以下を確認すること**：

1. ✅ **関数が定義されているか確認**
   - 呼び出し前に `function 関数名()` または `const 関数名 = () =>` が存在するか確認
   - `grep "function 関数名\|const 関数名"` で検索

2. ✅ **ビルド後にブラウザで動作確認**
   - ビルド後、実際にその機能を実行してエラーがないか確認
   - コンソールで `関数名 is not defined` エラーが出ないか確認

3. ✅ **複数箇所で呼び出す場合は特に注意**
   - PDF出力、CRM保存、Excelエクスポートなど、複数の処理から呼ばれる関数は特に注意
   - 呼び出し箇所をすべて確認
...
| エラー | 原因 | 対策 |
|--------|------|------|
| `preSaveSync is not defined` | 関数呼び出しを追加したが、関数本体を定義し忘れた | 呼び出しコードを追加する前に、必ず関数定義を追加する |
